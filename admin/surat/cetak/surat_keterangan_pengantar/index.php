<?php 
	include ('../../permintaan_surat/konfirmasi/part/akses.php');
  	...
  	$id = $_GET['id'];
  	$qCek = mysqli_query($connect,"SELECT penduduk.*, surat_keterangan_pengantar.* FROM penduduk LEFT JOIN surat_keterangan_pengantar ON surat_keterangan_pengantar.nik = penduduk.nik WHERE surat_keterangan_pengantar.id_skp='$id'");
  	while($row = mysqli_fetch_array($qCek)){
        ...
        foreach($qTampilDesa as $rows){
			...
			$id_pejabat_desa = $row['id_pejabat_desa'];
		  	$qCekPejabatDesa = mysqli_query($connect,"SELECT pejabat_desa.jabatan, pejabat_desa.nama_pejabat_desa FROM pejabat_desa LEFT JOIN surat_keterangan_pengantar ON surat_keterangan_pengantar.id_pejabat_desa = pejabat_desa.id_pejabat_desa WHERE surat_keterangan_pengantar.id_pejabat_desa = '$id_pejabat_desa' AND surat_keterangan_pengantar.id_skp='$id'");
...
<div>
	...
	<div id="isi3">
		...
		<table width="100%" style="text-transform: capitalize;">
			...
			<tr>
				<td class="indentasi">4. Warga Negara</td>
				<td>:</td>
			...
			<tr>
				...
				<td>:</td>
				<td><?php echo ucwords(strtolower($row['no_kk'])); ?></td>
			</tr>
			<tr>
				<td class="indentasi">10. Keperluan</td>
				<td>:</td>
				<td><?php echo strtoupper($row['keperluan']); ?></td>
			</tr>
			<tr>
				<td class="indentasi">11. Berlaku</td>
				<td>:</td>
				<td><?php echo ucwords(strtolower($row['berlaku'])); ?></td>
			</tr>
			<tr>
				<td class="indentasi">12. Golongan Darah</td>
				<td>:</td>
				<td style="text-transform: uppercase;"><?php echo $row['golongan_darah']; ?></td>
			</tr>
	...
	<table width="100%" style="text-transform: capitalize;">
		...
		<tr>
			...
			<?php
					...
					// Termasuk id_pejabat_desa dari tabel surat_keterangan_pengantar
					$qCek = mysqli_query($connect, "SELECT penduduk.*, surat_keterangan_pengantar.* FROM penduduk LEFT JOIN surat_keterangan_pengantar ON surat_keterangan_pengantar.nik = penduduk.nik WHERE surat_keterangan_pengantar.id_skp='$id'");
						...
						if (isset($pejabat_data[$id_pejabat_desa])) {
							...
							if ($id_pejabat_desa == 1) {
								...
								if (isset($pejabat_data[1])) {
									...
									echo '<img src="' . $url_gambar . '" alt="Barcode Pejabat" style="max-width: 100px;  margin-top: -84px">';
									echo "<br>";
								} else {
