...
<div class="modal fade" id="modalFormulir_<?php echo $row[$id_field]; ?>" tabindex="-1">
  ...
  for ($i = 1; $i <= 9; $i++) {
      ...
      if (!empty($data[$namaKey])) {
          echo '<tr>
                  <td style="width: 180px;">Pejabat ' . $i . '</td>
                  <td style="width: 10px;">:</td>
                  <td>' . htmlspecialchars($data[$namaKey]) . '</td>
                </tr>';
          echo '<tr>
                  <td>Jabatan ' . $i . '</td>
                  <td>:</td>
                  <td>' . htmlspecialchars($data[$jabatanKey]) . '</td>
                </tr>';
              }
          }
          ?>
          <tr>
            <td>Untuk</td><td>:</td><td><?= nl2br($data['untuk']) ?></td>
          </tr>
          <tr>
            <td>Tempat Tujuan</td><td>:</td><td><?= htmlspecialchars($data['tempat_tujuan']) ?></td>
          </tr>
          <tr>
            <td>Lama Penugasan</td><td>:</td><td><?= htmlspecialchars($data['lama_penugasan']) ?></td>
          </tr>
          <tr>
            <td>Tanggal Mulai</td><td>:</td><td><?= date('d-m-Y', strtotime($data['tanggal_mulai'])) ?></td>
          </tr>
          <tr>
            <td>Tanggal Selesai</td><td>:</td><td><?= date('d-m-Y', strtotime($data['tanggal_selesai'])) ?></td>
          </tr>
        </table>
