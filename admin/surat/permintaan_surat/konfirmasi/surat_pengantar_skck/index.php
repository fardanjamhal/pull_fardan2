                ...
                <div class="col-md-6">
                  <div class="box-body">
                   ...
                   <!-- FARDAN SALIN NOMOR OTOMATIS -->
                    <?php
                      // Ambil id_spskkb dari data surat sekarang (misal dari hasil query sebelumnya)
                      $current_id = isset($row['id_sps']) ? (int)$row['id_sps'] : 0;

                      if ($current_id > 0) {
                          // Query nomor surat sebelumnya berdasarkan id_spskkb yang kurang dari current_id
                          $query = mysqli_query($connect, "
                              SELECT no_surat 
                              FROM surat_pengantar_skck
                              WHERE id_sps < $current_id 
                              ORDER BY id_sps DESC 
                              LIMIT 1
                          ");
