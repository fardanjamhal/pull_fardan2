...
test("Startview: year view (integer)", function () {
  ...
  ok(
    picker.find(".datepicker-years").is(":not(:visible)"),
    "Years view hidden",
  );
  ok(
...
test("keepEmptyValues: none (default is false)", function () {
  ...
  equal(
    input_to.val(),
    "2016-03-27",
    "Input_from value should be distributed.",
  );
});
...
test("updateViewDate", function () {
  ...
  equal(
    monthShown.text(),
    "May 1945",
    "uses defaultViewDate on initialization",
  );
  ...
  equal(
    monthShown.text(),
    "June 1945",
    "changing month must still be possible",
  ); // and must trigger `changeMonth` event
